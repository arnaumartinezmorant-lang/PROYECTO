...
- DC1 `10.10.10.11` | DC2 `10.10.10.12` | SQL01 `10.10.10.21` | SQL02 `10.10.10.22`
  | Listener SQL `10.10.10.20` | BKP01 `10.10.10.31`
- Puertos: HTTPS 443 | SQL 1433 | LDAPS 636 | DNS 53 | Kerberos 88 | SMB 445 | replica AG 5022
  | RDP 3389 y WinRM 5985 (SOLO desde VLAN 30)
...
```
   ...
   [ DC1 ] [ DC2 ]  (AD DS + DNS, VLAN10)  <-- 636/88/53 desde WEB01/WEB02
   Administracion/DevOps (VLAN30 / VPN) ---3389/5985---> todos los servidores
```
Pasos:
...
5. Caja "Listener SQL 10.10.10.20". Flechas WEB01->Listener y WEB02->Listener con "1433".
6. SQL01 y SQL02 bajo el Listener; flecha doble entre ellos "AG replica 5022".
...
9. Caja "Administracion/DevOps (VLAN30/VPN)"; flecha hacia los servidores con "RDP 3389 / WinRM 5985".
...
```
     ...
     [Administracion/DevOps] --gestion (VPN, RDP/WinRM)-->
...
```
   ...
   [Balanceador / Proxy inverso]
        | HTTP/HTTPS
   [Servidor web/aplicacion (x2)] --LDAPS 636--> [Active Directory / DNS]
        | 1433
   [Acceso a datos (Listener)]
        |
...
| WEB01 .40.11|   |               |   | Listener .10.20 (1433)    |   |  RDP 3389      |
| WEB02 .40.12|   |               |   | BKP01 .10.31              |   |  WinRM 5985    |
+-------------+   +---------------+   +---------------------------+   +----------------+
...
- WEB01/WEB02 -> Listener(.10.20): **1433**
- WEB01/WEB02 -> DC1/DC2: **636 / 88 / 53**
- SQL01 <-> SQL02: **5022** (replica AG)
- Servidores -> BKP01: **445**
- VLAN30/VPN -> todos los servidores: **3389 / 5985** (y SOLO desde aqui)
Pasos:
...
4) WEBxx --consulta/escribe (1433)--> Listener --> SQL01 (activo)
5) SQL01 --replica (5022)--> SQL02     6) SQL01 --backup (445)--> BKP01 --> offsite cifrado
7) WEBxx --respuesta HTTPS--> LB01 --> Usuario
   (si WEB01 cae: el LB manda todo a WEB02; si SQL01 cae: el Listener apunta a SQL02)
```
